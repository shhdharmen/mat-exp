 ...
 * (`projects/ngm-dev/mat-expressive/src/lib/types/*.ts`) AND hand-copied
 * into `$known-attributes` / `$state-pseudo-map` in
 * `projects/ngm-dev/mat-expressive/src/lib/styles/utils/_constants.scss`.
 ...
 *     this script but checked against the live `MatExpressiveButtonState`
 *     union below — an un-mirrored rename throws here and fails the build
 *     immediately, instead of drifting quietly.
 *
 * Output:
 *   projects/ngm-dev/mat-expressive/src/lib/styles/utils/_generated-attributes.scss
 */
...
const LIB_ROOT = path.resolve(process.cwd(), 'projects/ngm-dev/mat-expressive');
...
const ATTRIBUTE_SOURCES: AttributeSource[] = [
  { scssKey: 'size', file: 'size.ts', typeName: 'MatExpressiveButtonSize' },
  { scssKey: 'shape', file: 'shape.ts', typeName: 'MatExpressiveButtonShape' },
  { scssKey: 'state', file: 'state.ts', typeName: 'MatExpressiveButtonState' },
  { scssKey: 'toggle', file: 'toggle.ts', typeName: 'MatExpressiveButtonToggle' },
  { scssKey: 'width', file: 'width.ts', typeName: 'MatExpressiveIconButtonWidth' },
  {
    ...
    // button-group's `MatExpressiveButtonGroupAppearance` (= Angular
    ...
    scssKey: 'appearance',
    file: 'appearance.ts',
    typeName: ['MatExpressiveIconButtonAppearance', 'MatExpressiveSplitButtonAppearance'],
  },
];

/**
 * Hand-authored `state -> CSS pseudo-class` mapping. Keys are validated
 * against the live `MatExpressiveButtonState` union at generation time (see
...
function assertStatePseudoMapInSync(stateValues: readonly string[]): void {
  ...
  const problems = [
    ...missingFromPseudoMap.map(
      (value) =>
        `  - "${value}" exists in MatExpressiveButtonState but has no STATE_PSEUDO_MAP entry`,
    ),
    ...staleInPseudoMap.map(
      (key) =>
        `  - "${key}" exists in STATE_PSEUDO_MAP but no longer exists in MatExpressiveButtonState`,
    ),
  ];
...
// attribute unions in \`projects/ngm-dev/mat-expressive/src/lib/types/\`.
...
// state -> CSS pseudo-class mapping, keyed by the live MatExpressiveButtonState
