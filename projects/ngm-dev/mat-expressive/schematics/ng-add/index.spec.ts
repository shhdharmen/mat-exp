...
describe('ng-add schematic', () => {
  ...
  it('inserts the @use/@include block before any existing rule', async () => {
    ...
    const ourUseIndex = content.indexOf("@use '@ngm-dev/mat-expressive'");
    const ourIncludeIndex = content.indexOf('mat-expressive.mat-expressive-all-styles()');
  ...
  it('inserts at the very top when the stylesheet has no leading @use statements', async () => {
    ...
    expect(content.indexOf("@use '@ngm-dev/mat-expressive'")).toBe(0);
  });
  ...
  it('is idempotent when the stylesheet already references the package', async () => {
    ...
    const occurrences = content.split('@ngm-dev/mat-expressive').length - 1;
  ...
  it('adds the prebuilt CSS as the first "styles" entry in angular.json for a CSS project, leaving the CSS file untouched, and is idempotent', async () => {
    ...
    expect(styles[0]).toBe('@ngm-dev/mat-expressive/styles.css');
    ...
    const occurrences = styles2.filter(
      (entry) => entry === '@ngm-dev/mat-expressive/styles.css',
    ).length;

    expect(occurrences).toBe(1);
    expect(styles2[0]).toBe('@ngm-dev/mat-expressive/styles.css');
  });
  ...
  it('emits only the selected component mixins (not all-styles) when components is a subset, and is idempotent', async () => {
    ...
    expect(content.split("@use '@ngm-dev/mat-expressive'").length - 1).toBe(1);
    expect(content).toContain('@include mat-expressive.mat-expressive-button-styles();');
    expect(content).toContain('@include mat-expressive.mat-expressive-icon-button-styles();');
    expect(content).not.toContain('mat-expressive-all-styles()');
    expect(content).not.toContain('mat-expressive-button-group-styles()');
    ...
    expect(secondContent.split("@use '@ngm-dev/mat-expressive'").length - 1).toBe(1);
  });
  ...
  it('falls back to "all" when every given component key is unknown', async () => {
    ...
    expect(content).toContain('@include mat-expressive.mat-expressive-all-styles();');
  });

  it('emits mat-expressive-all-styles() when components is "all" (unchanged existing behavior), and is idempotent', async () => {
    const tree = createWorkspaceTree();
    ...
    const occurrences =
      second.readContent('/src/styles.scss').split('@ngm-dev/mat-expressive').length - 1;
  ...
  it('mentions both the SCSS snippet and the CSS styles.css alternative when no global stylesheet is found', async () => {
    ...
    expect(warning).toContain("@use '@ngm-dev/mat-expressive'");
    expect(warning).toContain('@ngm-dev/mat-expressive/styles.css');
  });
});
