...
/**
 * `ng add @ngm-dev/mat-expressive` schematic.
 *
 ...
 *    - CSS projects: adds `@ngm-dev/mat-expressive/styles.css` as the first entry of the build
...
export default function ngAdd(options: NgAddOptions): Rule {
  return async (tree: Tree, context: SchematicContext) => {
    ...
    if (!configureStyles) {
      context.logger.info(
        'mat-expressive: skipping automatic style setup, as requested. You can configure ' +
    ...
    if (stylesheet.type === 'css') {
      if (options.components !== undefined) {
        context.logger.info(
          'mat-expressive: the "components" option is ignored for CSS projects — the prebuilt ' +
            '"@ngm-dev/mat-expressive/styles.css" stylesheet always includes every component.',
        );
      }
...
function printGettingStartedLink(context: SchematicContext): void {
  context.logger.info('');
  context.logger.info(`mat-expressive: for the full setup guide, see ${GETTING_STARTED_URL}`);
}
...
function verifyAngularMaterialInstalled(tree: Tree): void {
  ...
  if (!raw) {
    throw new Error(
      'mat-expressive: could not find a "package.json" at the workspace root; are you running ' +
  ...
  if (!hasMaterial) {
    throw new Error(
      'mat-expressive: "@angular/material" was not found in this workspace. Mat Expressive is ' +
        'built on top of Angular Material and requires it to be installed and configured first. ' +
        'Run "ng add @angular/material", then re-run "ng add @ngm-dev/mat-expressive".',
    );
  }
}
