...
export function insertStylesBlock(
  ...
  if (!raw) {
    context.logger.warn(`mat-exp: could not read "${stylesPath}".`);
    return;
  }

  const content = raw.toString('utf-8');
  if (content.includes('@ngm-dev/mat-exp')) {
    context.logger.info(`mat-exp: "${stylesPath}" already sets up Mat Expressive. Skipping.`);
    return;
  }
  ...
  context.logger.info(`mat-exp: updated "${stylesPath}" with the Mat Expressive styles.`);
}
...
export function buildIncludeBlock(components: ComponentSelection): string {
  ...
  const includes = components
    .map((key) => `  @include mat-exp.${COMPONENT_STYLE_MIXINS[key]}();`)
    .join('\n');
  return `html {\n${includes}\n}\n`;
}
...
export function buildManualInstructions(): string {
  ...
  return (
    'mat-exp: could not automatically locate a global stylesheet for this project ' +
