...
import { injectMatExpLoadingIndicatorOptions } from './loading-indicator.options';
import {
  MAT_EXP_LOADING_INDICATOR_SHAPES,
  MAT_EXP_LOADING_INDICATOR_VIEW_BOX,
 ...
 * `provideMatExpLoadingIndicatorOptions` or per-instance via the
 * `speed` input.
 *
...
@Component({
  ...
  host: {
    ...
    '[class]': 'matExpLoadingIndicatorClass',
    '(animate.enter)': 'onEnter($event)',
    '(animate.leave)': 'onLeave($event)',
  },
  templateUrl: './loading-indicator.html',
})
export class MatExpLoadingIndicator {
  private readonly _options = injectMatExpLoadingIndicatorOptions();
   ...
   * `slow`) – see {@linkcode MatExpLoadingIndicatorSpeed}.
   *
   ...
   * @default 'default'
   */
  public readonly speed = input<MatExpLoadingIndicatorSpeed>(this._options.speed ?? 'default');

  /** @internal */
  public readonly matExpLoadingIndicatorClass = 'mat-exp-loading-indicator';

  /**
   * Ordered list of SVG path `d` strings used by the shape morph loop.
   * @internal
   */
  public readonly shapes = MAT_EXP_LOADING_INDICATOR_SHAPES;

  /** @internal */
  public readonly viewBox = MAT_EXP_LOADING_INDICATOR_VIEW_BOX;
