...
import type { MatExpressiveButtonShape, MatExpressiveButtonSize } from '../../../types';

/** Minimal `ButtonGroupChild` fake that records every call it receives. */
class FakeButtonGroupChild implements ButtonGroupChild {
  size?: MatExpressiveButtonSize;
  shape?: MatExpressiveButtonShape;
 ...
 * call `bindButtonGroupChildren` - exactly how `MatExpressiveButtonGroup` and
 * `MatExpressiveSplitButton` call it from their own constructors.
 */
@Component({ standalone: true, template: '' })
class BroadcastHost {
  readonly children: WritableSignal<readonly ButtonGroupChild[]> = signal([]);
  readonly size: WritableSignal<MatExpressiveButtonSize | undefined> = signal(undefined);
  readonly shape: WritableSignal<MatExpressiveButtonShape | undefined> = signal(undefined);
