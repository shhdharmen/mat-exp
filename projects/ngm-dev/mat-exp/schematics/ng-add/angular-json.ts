...
/**
 * Adds `@ngm-dev/mat-exp/styles.css` as the first entry of the project's build target
...
export function addCssStylesheetEntry(
  ...
  const raw = tree.read('/angular.json');
  if (!raw) {
    context.logger.warn('mat-exp: could not read "angular.json".');
    return;
  }
  ...
  const project = name ? projects[name] : undefined;
  if (!project) {
    context.logger.warn('mat-exp: could not resolve a project in "angular.json".');
    return;
  }
  ...
  const buildTarget = targets?.['build'];
  if (!buildTarget) {
    context.logger.warn('mat-exp: could not find a "build" target in "angular.json".');
    return;
  }
  ...
  if (alreadyPresent) {
    context.logger.info(
      `mat-exp: "angular.json" already includes "${CSS_STYLES_ENTRY}". Skipping.`,
    );
    return;
  }
  ...
  context.logger.info(
    `mat-exp: added "${CSS_STYLES_ENTRY}" to the "styles" array in "angular.json".`,
  );
}
